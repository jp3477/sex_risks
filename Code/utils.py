...
class Utils: 
    ...
    def save_df(self, df, name):
        df.reset_index().drop('index', axis=1).to_feather(self.DATA_PATH+name+".feather")
        print("Saved",name)
        return True 
    ...
    def save_feature(self, data, name): 
        save_npz(self.DATA_PATH+'PSM_features/'+str(name)+'.npz', coo_matrix(data))
    ...
    def show(self, df): 
        with pd.option_context('display.max_rows', 50000, 'display.max_columns', 100):
            display(df)
    ...
    def print_table(self, df): 
        for _, info in df.iterrows(): 
            ...
            if info.logROR_avg>0: 
                print("{sex} \t {ror:.2f} ({low:.2f}, {upp:.2f}) \t {drug} \t {level} {name}".format(
                    sex = 'F',
                ...
                print("{sex} \t {ror:.2f} ({upp:.2f}, {low:.2f}) \t {drug} \t {level} {name}".format(
                    sex = 'M',
                    level = info.adr_class, 
                    name = info.adr_name, 
    ...
    def print_tex(self, df): 
        for _, info in df.iterrows(): 
            print()
            if info.logROR_avg>0: 
                print("{drug} & .. & .. & {name} & {sex} & {ror:.2f} & ({low:.2f}, {upp:.2f}) \\\ ".format(
                    sex = 'F',
                    name = info.adr_name, 
                    ror = info.logROR_avg,
                    upp = info.logROR_ci95_upp,
